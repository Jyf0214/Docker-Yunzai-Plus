...
本容器主要作为**逻辑端 (Client)** 运行。你需要配置它去连接协议端（如 NapCat）。

1.  启动容器等待初始化完成。
2.  在你的挂载目录 `yunzai-data/config/config/` 下找到 `qq.yaml` (如果没有需先运行一次生成)。
3.  修改配置连接到你的 NapCat：
...
bot:
  123456789:
    platform: 6 # 6 代表 WebSocket 适配器
    host: ws://你的NapCat地址:6099
```
